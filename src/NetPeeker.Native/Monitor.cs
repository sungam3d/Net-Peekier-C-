...
public sealed class Monitor : IDisposable
{
    ...
    public Monitor(TimeSpan? interval = null)
    {
        ...
        try
        {
            foreach (var exe in WfpFirewall.ListBlocked())
                if (!string.IsNullOrEmpty(exe) && !Settings.BlockedExes.Contains(exe))
                    Settings.BlockedExes.Add(exe);
        }
        catch
        {
            // Phase 3: WfpFirewall not implemented yet. Once it is, this
            // block reconciles any pre-existing block filters back into
            // settings on startup (so they don't silently vanish if the
            // settings.txt was deleted while filters remained).
        }
    }
    ...
    public void Dispose() { Stop(); Backend.Dispose(); }
    ...
    public void ApplySettings(bool syncFirewall = false)
    {
        ...
        if (syncFirewall)
        {
            // TODO (Phase 3): port monitor.apply_settings firewall sync.
            // Walk Settings.BlockedExes + tag-blocked, call WfpFirewall.BlockApp
            // for each; then per-IP rules and whitelists. See Python's
            // monitor.apply_settings for the canonical order.
        }
        Settings.Save();
    }
    ...
    {
        // TODO (Phase 3): port monitor._lockdown_sweep once WfpFirewall is
        // implemented. The logic is identical to Python's; only the
        // enforcement call changes (firewall.block_app -> WfpFirewall.BlockApp).
    }
        ...
        try
        {
            foreach (var exe in _lockdownBlocked.ToList())
                if (!Settings.BlockedExes.Contains(exe))
                    try { WfpFirewall.UnblockApp(exe); } catch { /* phase 3 */ }
        }
        catch { /* ignore */ }
